...
<div class="p-6 space-y-6">
    ...
    <div class="bg-white p-6 rounded-2xl shadow">
        <form method="GET" action="{{ route('dashboard.direktur') }}" class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            <div>
            ...
            <div>
                ...
                <select name="sales_id" class="mt-1 w-full border rounded p-2">
                    ...
                    <option value="404" {{ (string)$salesId === '404' ? 'selected' : '' }}>
                        Tidak Diteruskan
                    </option>

                    <option value="200" {{ (string)$salesId === '200' ? 'selected' : '' }}>
                        Diteruskan
                    </option>
                    @foreach($sales as $s)
            ...
            <div class="flex space-x-2">
                ...
                <a href="{{ route('dashboard.direktur') }}" class="bg-gray-300 px-4 py-2 rounded-lg hover:bg-gray-400">
                    Reset
                </a>
            </div>
        </form>
